...
TEST(Heap, pointerTestSmall) {
    Heap<int> heap;
    Heap<int>::Pointer ptr[5];
    for (int i = 0; i < 5; ++i) {
        ptr[i] = heap.insert(i);
    }
...
TEST(Heap, pointerTestBig) {
    Heap<int> heap;
    int sz = 1000;
    int arr[sz];
    Heap<int>::Pointer ptr[sz];
    for (int i = 0; i < sz; ++i) {
        arr[i] = i;
...
TEST(Heap, optimizedTimeTest) {
    int t = clock();
    Heap<int> heap;
    heap.optimize(10000000, 10000);
    int cnti = 0, cnte = 0;
    for (int i = 0; i < 10000000 + 10000; ++i) {
        if (rand() % 100 && !heap.is_empty() && cnte < 10000) {
            heap.extract_min();
            cnte++;
        }
        else {
